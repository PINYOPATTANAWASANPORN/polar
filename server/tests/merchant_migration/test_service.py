...
class TestImportCatalog:
    ...
    @pytest.mark.auth
    async def test_refresh_extra_price_on_imported_product_imports_as_dependency(
        self,
        mocker: MockerFixture,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        extra = next(item for item in items if item.source_id == "sub_extra")
        assert extra.status == PrecheckRecordStatus.importable
        assert extra.dependencies_imported is True
